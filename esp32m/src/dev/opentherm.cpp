...
#  define isOdd __builtin_parity
    ...
    esp_err_t Core::init(io::RmtRx *rx, io::RmtTx *tx, const char *name) {
      _name = name;
      _rx = rx;
      _tx = tx;
    ...
    esp_err_t Slave::init(io::RmtRx *rx, io::RmtTx *tx,
                          opentherm::ISlaveModel *model, const char *name) {
      ESP_CHECK_RETURN(Core::init(
          rx, tx, name ? name : (model ? model->name() : "ot-slave")));
      _model = model;
      if (_model)
        _model->init(this);
    ...
    void Slave::run() {
      ...
      for (;;) {
        ...
        delay(
            100);  // delay after the request accorting to OT spec - 20..800 ms
        transmit();
    ...
    /* --- MASTER --- */
    esp_err_t Master::init(io::RmtRx *rx, io::RmtTx *tx,
                           opentherm::IMasterModel *model, const char *name) {
      ESP_CHECK_RETURN(Core::init(
          rx, tx, name ? name : (model ? model->name() : "ot-master")));
      _model = model;
      if (!_model)
  ...
  namespace dev {
    ...
    OpenthermSlave::OpenthermSlave(io::RmtRx *rx, io::RmtTx *tx,
                                   opentherm::ISlaveModel *model,
                                   const char *name) {
      opentherm::Slave::init(rx, tx, model, name);
    }
    ...
    OpenthermMaster::OpenthermMaster(io::RmtRx *rx, io::RmtTx *tx,
                                     opentherm::IMasterModel *model,
                                     const char *name)
        : Device(Flags::HasSensors) {
      opentherm::Master::init(rx, tx, model, name);
    }
    ...
    OpenthermMaster *useOpenthermMaster(gpio_num_t rxPin, gpio_num_t txPin,
                                        opentherm::IMasterModel *model,
                                        const char *name) {
      auto rx = io::useRmtRx(rxPin);
      auto tx = io::useRmtTx(txPin);
      if (rx && tx)
        return new OpenthermMaster(rx, tx, model, name);
      if (rx)
        delete rx;
      if (tx)
        delete tx;
    ...
    OpenthermSlave *useOpenthermSlave(gpio_num_t rxPin, gpio_num_t txPin,
      ...
      auto tx = io::useRmtTx(txPin);
      if (rx && tx)
        return new OpenthermSlave(rx, tx, model, name);
      if (rx)
        delete rx;
      if (tx)
        delete tx;
